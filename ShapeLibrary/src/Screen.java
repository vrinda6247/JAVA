...
public class Screen {
	 ...
	 * @param point      for origin
	 * @param parameters for shape
	 * @return true if shape is successfully deleted from the list else false(shape
	 *         does not exist)
	 */
	public boolean deleteShape(ShapeType shapeType, Point point, List<Integer> parameters) {
		for (int i = 0; i < shapeList.size(); i++) {
			...
			if (shape.getShapeType().equals(shapeType) && shape.getOrigin().equals(point)
					&& shape.getParameters().equals(parameters)) {
