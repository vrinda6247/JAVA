...
public class TestPoly {
	...
	public void testAddPoly() {
		...
		assertArrayEquals(new Poly(new int[] { 4, 8, 1 }).getArray(),
				p.addPoly(new Poly(new int[] { 2, 4 })).getArray());
	}
	...
	public void testMultiplyPoly() {
		...
		assertArrayEquals(new Poly(new int[] { 4, 10, 16, 6 }).getArray(),
				p.multiplyPoly(new Poly(new int[] { 4, 2 })).getArray());
		assertArrayEquals(new Poly(new int[] { 1, 2, 5, 4, 6 }).getArray(),
				p2.multiplyPoly(new Poly(new int[] { 1, 2, 3 })).getArray());
	}
