...
public class Zoo {
	 ...
	 * @return function add zone in the zone list and returns the index of last zone
	 *         added
	 */
	...
	/**
	 * @param typeOfAnimal
	 * @return function add cages to the cage list and returns true if cage is added
	...
	public int indexOfZone(String type) {
		...
		for (int i = 0; i < zone.size(); i++) {
			...
			if (zone.get(i).getCategory().equals(category)) {
				...
				if (zone.get(i).cageList.contains(type)) {
					...
					if (zone.get(i).cageList.get(cageIndex).animal.size() < zone.get(i).cageList.get(cageIndex)
							.getCapacity()) {
						// return index of zone
						return i;
					}
		...
		// if zone does not exist and number of zones are less than zoo capacity then
		// add zone
